...
# open interface eth0 to send packets
  interface = popen('ifconfig | awk \'/h2-eth0/ {print $1}\'').read()

  for i in xrange(0,500):
# form the packet
    packets = Ether()/IP(dst="10.0.0.1",src=sourceIPgen())/TCP(dport=dst_port,sport=src_port)
    print(repr(packets))

# send packet with the defined interval (seconds) 
    sendp( packets,iface="h2-eth0",inter=0.025)
